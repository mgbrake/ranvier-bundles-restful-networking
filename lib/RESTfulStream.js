...
class RESTfulStream extends TransportStream
{
  ...
  login(username, state, callback) {
    const p = this.emit('login-restful', this, { name: username, state: state, socket: this.socket,
...
// commented  because when set the write function doesn't get called??
  ...
  write(message) {
    ...
    var htmlversion = convert.toHtml(message).replace(/(?:\r\n|\r|\n)/g, '<br />');
    const regexRemoveMapAndUnicode = /(?:(- . -)|(--*)|\x1b\[[0-9;]*[a-zA-Z])/g;
    var textonlyversion = message.replace(regexRemoveMapAndUnicode, '').replace(/(?:\r\n|\r|\n)/g, '\n');
    ...
    if(this._writemode==="html")
    {
      this.socket.write(
        ...
        htmlversion + //line breaks
        "</pre></div>"
      );
    }
    else if (this._writemode==="json")
    {
      if(message.includes("Connection taken over"))
      {
        return;
      }
      this.socket.write(
